...
public class CommunityController {
    ...
    //리뷰 하나 조회
    @GetMapping("/community/{userId}/{postId}")
    public ResponseEntity<CommunityResponseDto> getCommunity(
            @PathVariable Long userId,
            @PathVariable Long postId){
        CommunityResponseDto communityResponseDto = communityService.getCommunity(userId, postId);
    ...
    public ResponseEntity postCommunity(@PathVariable String type,
        ...
        CommunitySaveResponseDto communitySaveResponseDto = communityService.postCommunity(
                type, communityDto.getUserId(), communityDto.getTitle(), communityDto.getContent(), communityDto.getWriter(), image);
